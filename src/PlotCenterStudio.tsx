        ...
        <div className="min-h-screen p-6 max-w-[1400px] mx-auto text-slate-900">
            ...
            <div className="grid grid-cols-12 gap-6">
                ...
                <div className="col-span-3 space-y-6">
                    ...
                    <div className={cardClass}>
                        ...
                        {!image ? (
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-4 bg-white/50 p-5 rounded-3xl border border-white/80 shadow-sm">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1.5">
                                            <label className="text-[8px] font-black uppercase text-slate-400 px-1 tracking-widest">Ancho</label>
                                            <input type="number" value={targetW} onChange={(e) => setTargetW(Number(e.target.value))} className="w-full px-4 py-3 bg-white/50 border border-slate-100 rounded-2xl outline-none focus:border-brand-orange/30 text-xs font-bold transition-all" />
                                        </div>
                                        <div className="space-y-1.5">
                                            <label className="text-[8px] font-black uppercase text-slate-400 px-1 tracking-widest">Alto</label>
                                            <input type="number" value={targetH} onChange={(e) => setTargetH(Number(e.target.value))} className="w-full px-4 py-3 bg-white/50 border border-slate-100 rounded-2xl outline-none focus:border-brand-orange/30 text-xs font-bold transition-all" />
                                        </div>
                                    </div>
                                    <div className="flex gap-1 p-1 bg-slate-100/80 rounded-xl">
                                        {(['cm', 'm', 'in'] as const).map(u => (
                                            <button key={u} onClick={() => setUnit(u)} className={`flex-1 py-2 text-[9px] font-black rounded-lg transition-all ${unit === u ? 'bg-white text-slate-800 shadow-xl' : 'text-slate-400 hover:text-slate-600'}`}>{u.toUpperCase()}</button>
                                        ))}
                                    </div>
                                </div>
                    ...
                    {/* Vista de Marca */}
                    <div className="glass-card rounded-[3rem] p-8 hover-lift flex flex-col items-center">
                        <h2 className={sectionTitle}>VISTA DE MARCA</h2>
                        <div className="w-full h-44 rounded-[2rem] p-8 flex flex-col justify-between shadow-2xl relative overflow-hidden group">
                            {/* Mesh background effect */}
                            <div className="absolute inset-0 opacity-80" style={{ backgroundColor: selectedColor.hex }}></div>
                            ...
                            <div className="relative flex justify-between items-start">
                                <div className="w-10 h-10 rounded-xl border border-white/30 flex items-center justify-center backdrop-blur-sm">
                                    <div className="w-4 h-4 bg-white/80 rounded-full blur-[1px]"></div>
                                </div>
                                <div className="text-right">
                                    <p className="text-[7px] font-black text-white/40 uppercase tracking-[0.4em]">AUTHENTIC</p>
                                </div>
                            </div>

                            <div className="relative flex justify-between items-end">
                                <div>
                                    <p className="text-[11px] font-black text-white tracking-[0.1em]">PLOT CENTER</p>
                                    <p className="text-[7px] font-medium text-white/60 uppercase tracking-widest mt-0.5">Design Intelligence</p>
                                </div>
                                <div className="w-12 h-1 bg-white/30 rounded-full"></div>
                            </div>
                        </div>
                        <p className="text-[9px] text-slate-400 mt-6 font-semibold uppercase tracking-widest flex items-center gap-2">
                            BRAND ECOSYSTEM <span className="w-1.5 h-1.5 rounded-full bg-slate-200"></span>
                        </p>
                    </div>
                </div>
                ...
                <div className="col-span-6 space-y-6">
                    ...
                    {report && (
                        <div className="grid grid-cols-5 gap-6 pb-20">
                            <div className={cardClass + " col-span-2 !rounded-[3rem]"}>
                                <h3 className={sectionTitle}>NEURAL REPORT</h3>
                                <div className="text-[10px] text-slate-500 leading-relaxed font-semibold space-y-4 max-h-[300px] overflow-y-auto pr-4 scrollbar-hide">
                                    {report.analisis.split('\n').filter((p: string) => p.trim()).map((para: string, i: number) => (
                                        <p key={i} className="p-4 bg-white/30 rounded-2xl border border-white/50">{para}</p>
                                    ))}
                                </div>
                            </div>
                            <div className={cardClass + " col-span-3 !rounded-[3rem]"}>
                                <h3 className={sectionTitle}>BENCHMARKING INTEL</h3>
                                <div className="h-[250px] w-full pt-6">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={chartData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                                            <CartesianGrid strokeDasharray="6 6" stroke="#00000005" vertical={false} />
                                            <XAxis dataKey="name" fontSize={8} tickLine={false} axisLine={false} tick={{ fill: '#94a3b8', fontWeight: 900 }} />
                                            <YAxis fontSize={8} tickLine={false} axisLine={false} tick={{ fill: '#94a3b8', fontWeight: 900 }} />
                                            <Tooltip
                                                cursor={{ fill: '#f8fafc' }}
                                                contentStyle={{
                                            ...
                                            />
                                            <Bar dataKey="val" radius={[12, 12, 12, 12]} barSize={32}>
                                                {chartData.map((entry, index) => (
                                                    <RechartsCell key={`cell-${index}`} fill={entry.fill} fillOpacity={entry.name === 'Tu Archivo' ? 1 : 0.4} />
                                                ))}
                                            </Bar>
                                        </BarChart>
                                    </ResponsiveContainer>
                ...
                <div className="col-span-3 space-y-6">
                    ...
                    <div className={cardClass}>
                        <h2 className={sectionTitle}>
                            SELECCIÓN
                            <span className="text-[8px] bg-slate-900/5 text-slate-500 px-3 py-1 rounded-full font-black uppercase tracking-widest">PRO PICKER</span>
                        </h2>
                        <div className="w-full aspect-square rounded-[3rem] p-6 flex items-center justify-center mb-8 relative group">
                            <div className="w-full h-full rounded-[2.5rem] shadow-2xl transition-transform group-hover:scale-[1.02] duration-500" style={{ backgroundColor: selectedColor.hex }}></div>
                            <div className="absolute inset-0 bg-gradient-to-tr from-black/10 to-white/10 pointer-events-none rounded-[3rem]"></div>
                            <div className="absolute inset-0 border border-white/20 rounded-[3rem] pointer-events-none"></div>
                        </div>
                        <div className="space-y-4">
                            <div className="bg-white/40 p-5 rounded-3xl border border-white/60 flex items-center justify-between shadow-sm hover:shadow-md transition-all">
                                <div>
                                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">HEX CODE</p>
                                    <p className="text-2xl font-black text-slate-800 tabular-nums tracking-tighter">{selectedColor.hex}</p>
                                </div>
                                <button className="w-10 h-10 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-slate-300 hover:text-brand-orange hover:shadow-lg hover:shadow-brand-orange/10 transition-all" onClick={() => {
                                    navigator.clipboard.writeText(selectedColor.hex);
                                    showToast("HEX COPIADO");
                                }}><Copy size={18} /></button>
                            </div>
                            <div className="grid grid-cols-1 gap-3">
                                <div className="bg-white/40 p-4 rounded-2xl border border-white/60 cursor-pointer hover:bg-white/60 transition-all group" onClick={() => {
                                    navigator.clipboard.writeText(`${selectedColor.rgb.r}, ${selectedColor.rgb.g}, ${selectedColor.rgb.b}`);
                                    showToast("RGB COPIADO");
                                }}>
                                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 group-hover:text-brand-orange transition-colors">RGB SYSTEM</p>
                                    <p className="text-xs font-black text-slate-800 tabular-nums">{selectedColor.rgb.r}, {selectedColor.rgb.g}, {selectedColor.rgb.b}</p>
                                </div>
                                <div className="bg-white/40 p-4 rounded-2xl border border-white/60 cursor-pointer hover:bg-white/60 transition-all group" onClick={() => {
                                    ...
                                    <p className="text-[8px] font-black text-brand-orange/60 uppercase tracking-widest mb-1 group-hover:text-brand-orange transition-colors">CMYK PROCESS</p>
                    ...
                    <div className={cardClass}>
                        ...
                        <div className="space-y-8">
                            <div>
                                <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-3 italic">COMPLEMENTARY</p>
                                <div className="flex gap-3 h-10">
                                    {harmonies.complementary.map((c, i) => (
                                        <div key={i} className="flex-1 rounded-xl shadow-lg border-2 border-white/50 transition-transform hover:scale-105 cursor-pointer" style={{ backgroundColor: c }}></div>
                                    ))}
                                </div>
                            </div>
                            <div>
                                <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-3 italic">ANALOGOUS TRIO</p>
                                <div className="flex gap-3 h-10">
                            ...
                            <div>
                                <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-3 italic">TRIADIC BALANCE</p>
                                <div className="flex gap-3 h-10">
